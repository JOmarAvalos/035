...
<script type="text/javascript" src="<c:url value='/resources/js/lms/cursos/pregunta.js' />" type="text/javascript"></script>
<script
							...
							<c:forEach items="${lstRespuestas}" var="respuesta" varStatus="loop">
								<tr>
									...
									<td style="text-align: center;"> 
										<c:choose>
										    <c:when test="${respuesta.correcta == 1}">Si</c:when>
										    <c:when test="${respuesta.correcta == 0}">No</c:when>
									...
									<td style="text-align: center;">
										...
										<c:if test="${respuesta.banActivo == 1}">
											<a href="#" title="Estatus" onclick="estatusRespuesta(${curso.id}, ${actividad.id}, ${actividadPregunta.id}, ${respuesta.id});"><i class="fa fa-eye"></i></a>
										</c:if>
										<c:if test="${respuesta.banActivo == 0}">
											<a href="#" title="Estatus" onclick="estatusRespuesta(${curso.id}, ${actividad.id}, ${actividadPregunta.id}, ${respuesta.id});"><i class="fa fa-eye-slash"></i></a>
										</c:if>
										&nbsp;
										<c:if test="${loop.count != 1}">
											<a href="#" title="Arriba" onclick="orderArribaRespuesta(${curso.id}, ${actividad.id}, ${actividadPregunta.id}, ${respuesta.id});"><i class="fa fa-arrow-circle-up"></i></a> &nbsp; 
										</c:if>
				                    		<c:if test="${loop.count != fn:length(lstRespuestas)}">
											<a href="#" title="Abajo" onclick="orderAbajoRespuesta(${curso.id}, ${actividad.id}, ${actividadPregunta.id}, ${respuesta.id});"><i class="fa fa-arrow-circle-down"></i></a>&nbsp;
										</c:if>
									</td>
								</tr>
							</c:forEach>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content bg-glass-sknd">
			...
			<form role="form" id="formRespuesta"
				action="javascript:saveRespuesta(this, ${curso.id}, ${actividad.id},'${actividadPregunta.id}','${respuesta.id}');" method="POST"
