...
<script type="text/javascript" src="<c:url value='/resources/js/lms/cursos/preguntaEncuesta.js' />" type="text/javascript"></script>
	...
	<section class="content">
		<div>
			...
			<c:if test="${botonEstatus == 'HABILITADO'}">
				<button class="btn btn-primary" onclick="newRespuestaEncuesta();">Crear nueva respuesta</button>
			</c:if>
			...
			<div class="box-body table-responsive">
				...
				<table id="example1" class="table table-bordered table-striped">
					...
					<tbody>
						...
						<c:if test="${not empty lstRespuestasEncuesta}">
							<c:forEach items="${lstRespuestasEncuesta}" var="respuesta" varStatus="loop">
								<tr>
									...
									<td style="text-align: center;">
										<a href="#" title="Editar" onclick="editRespuestaEncuesta(${respuesta.id});"><i class="fa fa-gear"></i></a> 
										&nbsp; 
										<c:if test="${respuesta.banActivo == 1}">
											<a href="#" title="Estatus" onclick="estatusRespuestaEncuesta(${curso.id}, ${actividad.id}, ${encuestaPregunta.id}, ${respuesta.id});"><i class="fa fa-eye"></i></a>
										</c:if>
										<c:if test="${respuesta.banActivo == 0}">
											<a href="#" title="Estatus" onclick="estatusRespuestaEncuesta(${curso.id}, ${actividad.id}, ${encuestaPregunta.id}, ${respuesta.id});"><i class="fa fa-eye-slash"></i></a>
										</c:if>
										&nbsp;
										<c:if test="${loop.count != 1}">
											<a href="#" title="Arriba" onclick="orderArribaRespuestaEncuesta(${curso.id}, ${actividad.id}, ${encuestaPregunta.id}, ${respuesta.id});"><i class="fa fa-arrow-circle-up"></i></a> &nbsp; 
										</c:if>
				                    		<c:if test="${loop.count != fn:length(lstRespuestasEncuesta)}">
											<a href="#" title="Abajo" onclick="orderAbajoRespuestaEncuesta(${curso.id}, ${actividad.id}, ${encuestaPregunta.id}, ${respuesta.id});"><i class="fa fa-arrow-circle-down"></i></a>&nbsp;
										</c:if>
									</td>
								</tr>
							</c:forEach>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content bg-glass-sknd">
			...
			<form role="form" id="formRespuesta"
				action="javascript:saveRespuestaEncuesta(this, ${curso.id}, ${actividad.id},'${encuestaPregunta.id}','${respuesta.id}');" method="POST"
				class="" data-parsley-validate="">
				
				<div class="modal-header bg-glass-sknd">
					<h3 class="box-title" id="iptRespuestaTitulo">Nueva Respuesta Encuesta</h3>
				...
				<div class="modal-body text-white" style="color: grey;">
					<div class="box box-primary">
						...
						<div class="box-body">
							...
							<input type="hidden" class="form-control" id="iptIdPregunta" name="iptIdPregunta" value="${encuestaPregunta.id}">
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content bg-glass-sknd">
	        ...
	        <form role="form" id="formEstatusRespuesta" action="javascript:estatusRespuestaConfirmacion(this);" method="POST" class="" data-parsley-validate="" >
				...
				<div class="modal-header bg-glass-sknd">
					<h3 class="box-title">Respuesta Encuesta</h3>
					<button type="button" class="close" data-dismiss="modal"
						aria-label="Close">
						<span aria-hidden="true" style="color: white;">&times;</span>
					</button>
